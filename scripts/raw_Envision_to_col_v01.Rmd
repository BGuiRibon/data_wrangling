...
in_file <- c("20201130_091509_001_Plate1.xlsx",
		   "20201130_092209_001_Plate2.xlsx")

# prefix for name of assembled complete file
out_file <- c("20201130_AL_PlateReader_parsed_Plate1.tsv",
...
# keeping things
	time_zero <- unlist(sheet[which(sheet == "Measurement date", arr.ind = TRUE)[1,1]+1,
	                          which(sheet == "Measurement date", arr.ind = TRUE)[1,2] ], use.names = F)
 	time_zero <- as.POSIXct(as.Date("1900-01-01 00:00", tzone="UTC")) + 3600*5 + 3600*24*(as.numeric(time_zero)-2)
 	measurement <- unlist(sheet[col_info$start-2, 1], use.names = F)

# call mapply to get back all drug treatments for this sheet
col_data <- mapply(col_extraction, col_info$start, col_info$end, measurement, MoreArgs = list(sheet), SIMPLIFY = T)

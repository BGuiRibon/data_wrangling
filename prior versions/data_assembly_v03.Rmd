...
# merging of different multiple different cell line or drug treated plates
  - With Jason's 20201415_JM Exp_ID done manually during analysis
  - in future versions will be taken care of during import/assembly with meta data
  - see 7) above
...
# append column to well_level dataframe
     if (type=="numeric") { 
               ...
               well_level <- cbind(well_level, as.factor(values))
     }
...
new_order <- c("Experiment_ID", "Plate_ID", "Plate_No", "Plate_Row", "Plate_Col", "Well_ID", "Well_No", exp_level_names, plate_level_names) 
...
# see if there are overlapping row names
intersect(names(meta), names(raw))
